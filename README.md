...
5. Если активное приложение — сам Codex (`com.openai.codex`), скрипт помечает `id` обработанным и ничего не вставляет: Codex уже вставляет текст сам, иначе получится дубль.
6. Для остальных приложений отправляет `Cmd+V` через `key code 9` (раскладко-независимо)
7. Скрипт завершается. Никакого фонового процесса до следующей диктовки.
